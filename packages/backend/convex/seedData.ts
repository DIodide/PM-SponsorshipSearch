...
// Mutation to seed the database with sample teams
export const seedTeams = mutation({
  ...
  handler: async (ctx) => {
    ...
    return { message: "Database seeded successfully", count: ids.length };
  },
});

// Mutation to clear and reseed the database
export const clearAndReseed = mutation({
  ...
  handler: async (ctx) => {
    ...
    return { message: "Database cleared and reseeded", count: ids.length };
  },
});
