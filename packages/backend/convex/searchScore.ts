...
import { computeBrandSimilarity } from "./similarityScoring";   // your module

// ------------------------------------------------------------
// Existing createSession, updateSessionStatus, getSession, etc.
...
// YUBI: should not need or use this function
...
// ----------------------------------------------------------------------
// NEW: The main search executor (replaces web-search with similarityScoring)
// ----------------------------------------------------------------------
...
export const runSearch = action({
  ...
  handler: async (ctx, args) => {
    const { sessionId, query, filters } = args;

    // Mark the session as "running"
    await ctx.runMutation("sessions:updateSessionStatus", {
      sessionId,
      status: "running"
    });

    // Step 1. Load teams (raw DB teams)
    const teams = await ctx.runQuery("teams:getAll");
    ...
    // Returns:  Array<{ name: string; similarity_score: number; ... }>
    const scoredTeams = await computeBrandSimilarity(
        // needs specific type of arguments
      query, filters
    );

    // Step 3. Ensure results are sorted descending (defensive)
    scoredTeams.sort((a, b) => b.similarity_score - a.similarity_score);

    // Step 4. Persist or update session results
    await ctx.runMutation("sessions:updateSessionResults", {
      sessionId,
      results: scoredTeams.map(team => ({
        name: team.name,
        similarity_score: team.similarity_score
      }))
    });
